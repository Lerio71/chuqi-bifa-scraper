...
# 20 家公司 sid（欧赔/亚盘变化页）
SIDS = [("51_2","竞彩"),("1","Crow*"),("2","澳*"),("4","12B*"),("5","*365"),("6","易**"),
...
def build_urls(match_id):
    ...
    for sid, name in SIDS:
        urls.append((f"spf_{sid}", f"{LIVE}info-new-list-spf/{match_id}-{sid}-0-3/"))
        urls.append((f"rq_{sid}", f"{LIVE}info-new-list-rq/{match_id}-{sid}-0-1/"))
    return urls
...
def extract_match(match_id, outdir, workers=12, use_cache=True):
    ...
    # 2) 20 家变化页解析
    for sid, name in SIDS:
        row = {"name": name, "spf": None, "rq": None}
        for ptype in ["spf", "rq"]:
            txt = html_store.get(f"{ptype}_{sid}")
            if txt:
                title, headers, rows = parse_change_table(txt)
                row[ptype] = {"title": title, "headers": headers, "rows": rows, "n": len(rows)}
